...
We have seen that (multiple) imputation is straightforward with `mice`. However, don't let the simplicity of the software fool you into thinking that the problem itself is also straightforward. In the next practical we will therefore explore how the mice package can flexibly provide us the tools to assess and control the imputation of missing data.
